// Generated from generators/uShapeOverheadCabinet/generator.ts - do not edit.
...
// generators/uShapeOverheadCabinet/generator.ts
var DEFAULT_ARM_DEPTH = 350;
var MIN_ARM_RUN = 150;
...
function mapBoard(board, pose, prefix) {
  ...
  if (pose.rotZ === 0) {
    next.profileVector = (board.profileVector || []).map((p) => {
      if ("x" in p && "y" in p) return { x: p.x + pose.x, y: p.y + pose.y };
      if ("x" in p && "z" in p) return { x: p.x + pose.x, z: p.z };
      if ("y" in p && "z" in p) return { y: p.y + pose.y, z: p.z };
      return p;
    });
  } else {
    next.profileVector = void 0;
    next.cutProfileVector = void 0;
  }
  return next;
}
function defaultZones(width) {
  return [{ id: "zone-1", type: "up_flap", width: round1(width) }];
}
function armParams(params, width, zones) {
  return {
...
function generateUOverheadCabinet(params) {
  const errors = [];
  const warnings = [];
  ...
  if (W < 2 * arm + MIN_ARM_RUN) errors.push(`cabinetWidth must be at least 2 \xD7 armDepth + ${MIN_ARM_RUN} mm so the U has an opening.`);
  const empty = () => ({
    params: {
      cabinetWidth: W,
      cabinetDepth: D,
  ...
  for (const [name, r] of [["back", back], ["left", left], ["right", right]]) {
  ...
  const features = [
    ...back.features.map((f) => ({ ...f, source: "back" })),
    ...left.features.map((f) => ({ ...f, source: "left" })),
    ...right.features.map((f) => ({ ...f, source: "right" }))
  ];
  return {
    params: {
      cabinetWidth: W,
