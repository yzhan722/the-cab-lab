...
var DEFAULT_SIDE_PANEL_OPTIONS = {
  panelType: "carcass",
  frontVisible: false,
...
function normalizeSidePanelOptions(input) {
  return {
...
function sidePanelThickness(options, cpt, fpt) {
  return options.panelType === "door" ? fpt : cpt;
}
function kitchenStructuralDepth(settings) {
  return Math.max(0, n(settings.depth, 0) - n(settings.frontThickness, 16));
}
function resolveOuterSideOptions(state, side, warnings) {
  ...
  const preferredZone = zonesWithOptions.find((zone) => isDoorZone(zone.zoneType)) || zonesWithOptions.find((zone) => zoneVisible(zone.zoneType)) || zonesWithOptions[0];
  return normalizeSidePanelOptions(preferredZone?.[key]);
}
function sideReferences(state, warnings) {
...
function board(id, name, type, category, materialThickness, profilePlane, thicknessAxis, bounds, notches, notes) {
  return {
    id,
    name,
    type,
...
function functionalProfileXY(clearX0, clearX1, y0, y1, leftTongueLength, rightTongueLength, tongueY0, tongueY1) {
  const raw = [
    [clearX0, y0],
    [clearX1, y0],
    [clearX1, tongueY0],
...
function isCollinear(a, b, c) {
  const sameX = Math.abs(a[0] - b[0]) <= 1e-3 && Math.abs(b[0] - c[0]) <= 1e-3;
  const sameY = Math.abs(a[1] - b[1]) <= 1e-3 && Math.abs(b[1] - c[1]) <= 1e-3;
  return sameX || sameY;
}
function trimClosedProfile(points) {
  const deduped = [];
...
function auditClosedProfile(raw, cleaned) {
  ...
  if (Math.abs(area) <= 1e-3) warnings.push("Profile area is zero.");
  return {
...
function vProfileWithWheel(base, wheel, cd, cpt, c) {
...
function applySideFrontVisibility(profile, options, fpt, bch, frontRepairMaxY, frontRepairMaxZ, cabinetHeight) {
  ...
  const notchlessTop = profile.map(([y, z]) => [
    z >= cabinetHeight - frontRepairMaxZ - 1e-3 && y >= -1e-3 && y <= frontRepairMaxY + 1e-3 ? -fpt : y,
    z >= cabinetHeight - frontRepairMaxZ - 1e-3 && y >= -1e-3 && y <= frontRepairMaxY + 1e-3 ? cabinetHeight : z
  ]);
  const extended = notchlessTop.map(([y, z]) => [
    Math.abs(y) <= 1e-3 && z >= frontRepairMaxZ - 1e-3 && z < cabinetHeight - 1e-3 ? -fpt : y,
    z
  ]);
  if (options?.bchNotchEnabled !== false) return trimClosedProfile(extended);
  return trimClosedProfile(extended.map(([y, z]) => [
    z <= frontRepairMaxZ + 1e-3 && y >= -1e-3 && y <= frontRepairMaxY + 1e-3 ? -fpt : y,
    z
  ]));
}
function computeXBoundaries(columns) {
  const boundaries = [0];
...
function computeVPanels(state, constants, warnings) {
  const g = state.globalSettings;
  const cw = n(g.length, 0);
...
function computeColumns(state, vPanels) {
  const boundaries = computeXBoundaries(state.columns);
...
function computeZones(state, columns) {
...
function removeFrontTopReceiverNotch(profile, cd, ch, cpt, constants) {
  const na = cpt + constants.notchAllowanceExtra;
  const r = constants.receiverNotchDepth;
  return profile.map(([y, z]) => {
    if (Math.abs(z - (ch - na)) <= 1e-3 && y >= -1e-3 && y <= r + 1e-3) return [0, z];
    if (Math.abs(z - ch) <= 1e-3 && y >= -1e-3 && y <= r + 1e-3) return [0, z];
    return [y, z];
  });
}
function removeUnsupportedEdgeStoveVPanelNotches(vPanels, zones, state, constants) {
  const g = state.globalSettings;
  const cd = kitchenStructuralDepth(g);
  const ch = n(g.height, 0);
  const cpt = n(g.materialThickness, 15);
...
function supportStripNotches(idPrefix, strip, vPanels, cpt, c, kind) {
  const notchDepth = c.supportStripNotchDepth;
  return vPanels.filter((v) => v.centerX >= strip.x0 - 0.5 && v.centerX <= strip.x1 + 0.5).map((v) => {
    ...
    if (kind === "T1") return { ...base, y0: strip.y1 - notchDepth, y1: strip.y1, from: "rear" };
    if (kind === "T2") return { ...base, y0: strip.y0, y1: strip.y0 + notchDepth, from: "front" };
    if (kind === "T3") return { ...base, z0: strip.z0, z1: strip.z0 + notchDepth, from: "bottom" };
    return { ...base, z0: strip.z1 - notchDepth, z1: strip.z1, from: "top" };
  });
}
function splitStripForStove(strip, stoveCuts, constants) {
  let segments = [{ ...strip, segmentIndex: 0 }];
...
function splitStripByXCuts(strip, xCuts, constants) {
  let segments = [{ ...strip, segmentIndex: 0 }];
...
function validWheelAvoidances(state, cd, ch) {
  return (state.wheelAvoidances || []).map((avoidance, index) => {
...
function generateBaseBoards(state, vPanels, columns, zones, constants, warnings) {
  const g = state.globalSettings;
  const cw = n(g.length, 0);
  const cd = kitchenStructuralDepth(g);
  const ch = n(g.height, 0);
  ...
  const stoveCuts = zones.filter((zone) => zone.zoneType === "stove").map((zone) => {
  ...
  splitStripByXCuts(b4Base, wheelAvoidances, constants).forEach((segment) => {
    boards.push(board(
      ...
      wheelAvoidances.length ? ["Lower B4 kept outside wheel avoidance span."] : void 0
    ));
  });
...
function boundaryBoardType(zoneType, isBottomZone) {
...
function addFunctionalBoard(boards, id, type, column, centerZ, cpt, cd, constants, notes) {
...
function generateFunctionalBoards(columns, zones, state, constants, warnings) {
...
function shortenHorizontalBoardsForWheelAvoidance(boards, state, warnings) {
  const g = state.globalSettings;
  const cd = kitchenStructuralDepth(g);
  const ch = n(g.height, 0);
  const wheelAvoidances = validWheelAvoidances(state, cd, ch);
  ...
  const candidates = boards.filter(
    (item) => (item.type === "drawer_divider" || item.type === "full_depth_shelf" || item.type === "door_shelf") && item.profilePlane === "XY"
  );
  for (const boardItem of candidates) {
    ...
    for (const avoidance of wheelAvoidances) {
      ...
      if (intersects(boardItem.z0, boardItem.z1, 0, avoidance.height)) {
        ...
        if (boardItem.y1 > clearanceY + 1e-3) {
...
function addSideStrengtheningStrips(boards, columns, zones, state, warnings) {
  const g = state.globalSettings;
  const cpt = n(g.materialThickness, 15);
  const bch = n(g.bottomClearanceHeight, 0);
  const ch = n(g.height, 0);
  const refs = sideReferences(state, warnings);
  const addStrip = (side, columnIndex, zone) => {
    ...
    const belowZone = columnZones.find((candidate) => Math.abs(candidate.z1 - zone.z0) <= 1e-3);
    const aboveZone = columnZones.find((candidate) => Math.abs(candidate.z0 - zone.z1) <= 1e-3);
...
function applySideStrengtheningStripShelfJoinery(boards, cpt) {
  const notchY0 = 0;
  const notchY1 = 85;
...
function slotRequestForBoardEnd(boardItem, vPanel, side, oppositeVisible, cpt, constants) {
  ...
  const slotType = hiddenOuterGroove ? "half" : oppositeVisible ? "half" : "through";
  ...
  return {
    ...
    slotType,
    tongueLength,
    grooveDepth: slotType === "half" ? panelThickness / 2 : void 0,
    x0: vPanel.x0,
    x1: vPanel.x1,
    y0: Math.max(0, y0),
    y1: slotY1,
...
function generateSlotRequests(boards, columns, zones, vPanels, state, constants) {
  const cpt = n(state.globalSettings.materialThickness, 15);
  const requests = [];
...
function resolveSlots(requests, preferences, warnings, errors) {
  const resolved = requests.map((request) => ({ ...request, resolvedSlotType: request.slotType }));
  const resolvedTypeForMode = (mode, side) => {
  ...
  for (const panelIndex of panelIndexes) {
    const panelRequests = resolved.filter(
      (request) => request.vPanelIndex === panelIndex && request.slotType === "half"
    );
    ...
    const mode = preference || (hasLeft ? "left_face_half_allowed" : hasRight ? "right_face_half_allowed" : void 0);
    if (!mode) continue;
...
function updateFunctionalBoardProfilesFromSlots(boards, resolvedSlots, cpt, cd, constants) {
  const tongueForSlot = (slot) => {
    if (!slot) return 0;
...
function rectangleOuter(x0, x1, y0, y1) {
  return [[x0, y0], [x1, y0], [x1, y1], [x0, y1], [x0, y0]];
}
function boardPlaneAxes(plane) {
  return plane === "YZ" ? { hKey: "y", vKey: "z" } : plane === "XZ" ? { hKey: "x", vKey: "z" } : { hKey: "x", vKey: "y" };
}
function notchCutoutForBoard(boardItem, notch) {
...
function edgeForCutout(cutout, v0, v1) {
  if (Math.abs(cutout.y0 - v0) <= 1e-3 && cutout.y1 > v0) return "min";
  if (Math.abs(cutout.y1 - v1) <= 1e-3 && cutout.y0 < v1) return "max";
  return null;
}
function outerWithEdgeNotches(h0, h1, v0, v1, cutouts) {
  const edgeNotches = cutouts.map((cutout) => ({ cutout, edge: edgeForCutout(cutout, v0, v1) })).filter((item) => Boolean(item.edge));
  ...
  const edge = edges[0];
  const notches = edgeNotches.map(({ cutout }) => ({
    x0: Math.max(h0, cutout.x0),
    x1: Math.min(h1, cutout.x1),
    y0: Math.max(v0, cutout.y0),
    y1: Math.min(v1, cutout.y1),
    id: cutout.id
  })).filter((notch) => notch.x1 > notch.x0 && notch.y1 > notch.y0).sort((a, b) => a.x0 - b.x0);
...
function slotCutoutForBoard(boardItem, slot) {
...
function buildBoardBodies(boards, resolvedSlots) {
  for (const boardItem of boards) {
    ...
    const notchCutouts = (boardItem.notches || []).map((notch) => notchCutoutForBoard(boardItem, notch)).filter((cutout) => Boolean(cutout));
    ...
    const outer = usesCustomOuter ? boardItem.profileXY : edgeProfile.outer;
    const slotCutouts = resolvedSlots.filter((slot) => slot.boardId === boardItem.id).map((slot) => slotCutoutForBoard(boardItem, slot)).filter((cutout) => Boolean(cutout));
    boardItem.body = {
...
function buildVPanelBodies(vPanels, resolvedSlots) {
  for (const panel of vPanels) {
    panel.body = {
      plane: "YZ",
      outer: panel.yzProfile,
      cutouts: resolvedSlots.filter((slot) => slot.vPanelIndex === panel.index && slot.resolvedSlotType !== "none").map((slot) => ({
...
function buildPanelBodies(boards, vPanels, resolvedSlots) {
  buildBoardBodies(boards, resolvedSlots);
  buildVPanelBodies(vPanels, resolvedSlots);
}
function boardDxfEntry(boardItem) {
...
function vPanelDxfEntry(panel, cpt) {
...
function buildPanelDxf(boards, vPanels, cpt) {
  return [
    ...boards.map((boardItem) => boardDxfEntry(boardItem)),
    ...vPanels.map((panel) => vPanelDxfEntry(panel, cpt))
  ];
}
var FRONT_PANEL_ZONE_TYPES = /* @__PURE__ */ new Set(["left_door", "right_door", "double_door", "drawer", "down_flap"]);
function isFrontPanelZone(type) {
...
function columnHasAnyFrontPanel(column) {
  return Boolean(column?.zones?.some((zone) => isFrontPanelZone(zone.zoneType)));
}
function upperDividerCenterZ(zone, state) {
  const ch = n(state.globalSettings.height, 0);
  return Math.abs(zone.z1 - ch) <= 1e-3 ? ch - n(state.globalSettings.materialThickness, 15) / 2 : zone.z1;
}
function buildFrontPanels(columns, zones, state, warnings) {
  const g = state.globalSettings;
  const cw = n(g.length, 0);
  const ch = n(g.height, 0);
  const cpt = n(g.materialThickness, 15);
  const fpt = n(g.frontThickness, 16);
  const fc = n(g.frontClearance, 2.5);
  const lockGlobalEnabled = g.lockEnabled !== false;
  ...
  for (const column of columns) {
    ...
    const baseX0 = column.index === 0 ? fc : columnHasAnyFrontPanel(leftColumn) ? column.logicalX0 + fc / 2 : column.logicalX0 - cpt / 2;
    const baseX1 = column.index === columns.length - 1 ? cw - fc : columnHasAnyFrontPanel(rightColumn) ? column.logicalX1 - fc / 2 : column.logicalX1 + cpt / 2;
    ...
    for (const zone of columnZones) {
      ...
      const aboveZone = columnZones.find((candidate) => Math.abs(candidate.z0 - zone.z1) <= 1e-3);
      const belowZone = columnZones.find((candidate) => Math.abs(candidate.z1 - zone.z0) <= 1e-3);
      const z1 = Math.abs(zone.z1 - ch) <= 1e-3 ? ch - fc : isFrontPanelZone(aboveZone?.zoneType) ? zone.z1 - fc / 2 : zone.z1 + cpt / 2;
      const z0 = Math.abs(zone.z0 - n(g.bottomClearanceHeight, 0)) <= 1e-3 ? bottomZ : isFrontPanelZone(belowZone?.zoneType) ? zone.z0 + fc / 2 : zone.z0 - cpt / 2;
      const width = x1 - x0;
      const height = z1 - z0;
      const seamX = (x0 + x1) / 2;
      const leaves = zone.zoneType === "double_door" ? [
        { idSuffix: "-left", type: "left_door", x0, x1: seamX - fc / 2 },
        { idSuffix: "-right", type: "right_door", x0: seamX + fc / 2, x1 }
      ] : [{ idSuffix: "", type: zone.zoneType, x0, x1 }];
      const hingeSettings = zone.hingeSettings || {};
      ...
      for (const leaf of leaves) {
        ...
        const zoneLockEnabled = zone.lockEnabled !== false;
        if (lockGlobalEnabled && zoneLockEnabled) {
          const lockSideCenterOffset = n(zone.lockSideCenterOffset, 80);
          const lockCenterX = leaf.type === "left_door" ? leaf.x1 - lockSideCenterOffset : leaf.type === "right_door" ? leaf.x0 + lockSideCenterOffset : (leaf.x0 + leaf.x1) / 2;
...
function svgFront(columns, zones, state, boards, frontPanels) {
...
function svgBoardTop(boardItem) {
  if (!boardItem) return "";
...
function generateKitchenCabinetGeometry(rawState) {
...
export {
  generateKitchenCabinetGeometry
};
