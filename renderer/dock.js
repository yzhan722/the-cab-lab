...
// (dock per pane, sizes, collapsed state) is remembered in localStorage.
//
...
function load() {
  ...
  try {
    ...
    raw = null; // corrupt layout: fall back to the built-in one
  }
...
function save() {
  ...
  } catch (_) { /* private mode: the layout just does not persist */ }
}
...
function clampSize(zone, v) {
  ...
  // The 3D view keeps VIEWPORT_MIN whatever the module rail and the other side
  // dock already take.
...
function render() {
  for (const zone of ZONES) {
    ...
    for (const id of ids) {
      // Re-parent only on a real move: appending a node that is already there
      // would take the focus out of a field the user is typing in.
...
function measureDrops() {
  ...
  const rect = (zone) => {
    const dock = docks[zone];
    // A hidden or collapsed dock is too thin to aim at: use a full-size strip.
...
function bindResize(zone) {
  ...
  handle.addEventListener("pointerdown", (e) => {
    ...
    const move = (ev) => {
      ...
      // The handle sits on the edge facing the viewport, so it grows the dock
      // when dragged away from that edge.
